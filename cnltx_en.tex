...
\cnltxbase, \cnltxlistings, \cnltxtools, \needpackage{translations},
...
code.  The predifined style has many options that will not be mentioned here.
...
\begin{sourcecode}
  ...
  % activate my microtype settings for tracking of small caps
  % with \microtypesetup{tracking=scshape}
  % needs files from https://bitbucket.org/cgnieder/microtype-config/
  \ifboolexpr{not bool{xetex} and not bool{luatex}}
...
\subsubsection{Sourcecode}\label{sec:listings-sourcecode}
...
\begin{commands}
  ...
  \command{cnltx\at ifisnum}[\marg{token list}\marg{true}\marg{false}]
  ...
  \command{cnltx\at fullexpand\at afterarg}[\marg{cs}\marg{argument}\marg{code}]
    ...
    \meta{cs}\marg{argument} in the input stream (\eg\ for expanding
