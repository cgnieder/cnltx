...
The \cnltx\ bundle contains different packages and classes.  I developed it as
a successor of my class \cls{cnpkgdoc} that I used for writing the
...
classes.  This is actually a bit of a contradiction: the document class loads
all packages of the bundle which makes it more feature-rich than
\cls{cnpkgdoc}.  The bundle provides source code environments that also print
the output and defines quite a lot of macros for formatting of control
sequence names, package names, package options and so on.
...
The best documentation for the bundle is as always the source code of the
...
\begin{itemize}
  \item \cnltxbase\ -- a package that defines base macros for error-messaging,
    ...
    other packages and classes of the \cnltx\ bundle load this package.  This
    package can be used stand-alone. \\
...
\begin{options}
  ...
  \keybool{side-by-side}\Default{false}
    ...
    (think of \cs*{parindent}, page breaks \ldots).  This option has no effect
...
command that calls an associated internal command (\cs*{@arrabic} in the case
...
argument.  The command sequence \cs*{c@\meta{counter}} is the count (in the \TeX\
sense) that is associated with the counter \meta{counter}, \ie, it holds the
actual number.  The command \cs*{@arabic} now simply typesets the integer
value of the count.
...
\begin{commands}
  \command{newexpandablecmd}[\sarg\marg{cs}\oarg{num args}\oarg{default
    ...
    mandatory arguments it may be necessary to add a trailing \cs*{empty}.  If
    you define a command \emph{without} optional argument this command falls
    back to \cs*{newcommand}.
  ...
  \ifboolexpr
    {
      ...
      test {\ifcsdef{MT@pr@set@@romansans}}
    }
    {}
    {
      \DeclareMicrotypeSet{romansans}{
...
\pkg{biblatex}'s \code{indexing} option is used.  \emph{The styles can only be
  used with the \cnltxdoc\ class} as it relies on definitions made by it.
...
\begin{commands}
  ...
  \expandable\command{iftest}[\marg{test directive}\marg{true}\marg{false}]
    ...
    places \meta{true} or \meta{false} in the input stream.  Test directive
    should be a \TeX\ test like \cs*{ifx}\meta{token1}\meta{token2}.\\
  ...
  \command{cnltx\at expandargs}[\darg{specs}\meta{control sequence}]
    ...
    \code{N} means unexpanded token, \code{n} means unexpanded braced group,
    \code{c} means braced group converted into a control sequence name,
    \code{o} means braced group expanded once, \code{f} means braced group
    expanded with \cs*{romannumeral}, and \code{x} means braced group expanded
    with \cs*{edef}.
...
All entries in \code{cnltx.bib} are entries of the \code{@package} type.
The cite keys that the file currently contains are listed below.
