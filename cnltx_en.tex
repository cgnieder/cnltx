...
\setcnltx{
  ...
  add-cmds = {
    changedversion,
    cnltx@define@colorscheme,cnltxat,cnltxbang,cnltxequal,
    ...
    oarg,
    opt,option,
    sarg,setcnltx,sinceversion
  },
  add-silent-cmds = {at,carlisle,foothree} ,
  add-envs = {
    commands,
    environments,
    example,
...
\newnote*\bypackage{provided by the \cnltxexample\ package}
...
\begin{commands}
  ...
  \command{key}[\sarg\code{-}\marg{name}\marg{values}]
    A key \meta{name} with values \meta{values}, the optional star prevents an
    index entry, the  optional \code{-} strips the braces around \meta{value};
    \cs{key}\code{\{key\}}\code{\{value\}}: \key{key}{value};
    \cs{key}\code{-}\code{\{key\}}\code{\{value\}}: \key-{key}{value}
...
\begin{commands}
  ...
  \command{cnltx\at ifdash}[\marg{true}\marg{false}]
    A wrapper for \lstinline[style=cnltx]+\cnltx@ifsym{-}+.
...
\begin{commands}
  ...
  \command{cnltxat}
    ...
    \lstinline[style=cnltx]+\def\at{\cnltxat}+.
...
\begin{commands}
  ...
  \command{cnltx\at predefined\at environments}
    A comma-separated list of predefind `silent' environment names.
\end{commands}
